  ...
  <div class="puzzle-container" :class="status">
    <div class="puzzle-header">
      ...
      <div>
        ...
        <span
          class="close-btn iconfont icon-close2"
    ...
    <div class="puzzle-content">
      ...
      >
        <span
          class="iconfont"
          :class="verification ? 'icon-success' : 'icon-close'"
        ></span>
        <span>{{
          verification ? '验证通过' : '验证失败，请移动到正确位置！'
        }}</span>
      </p>
    </div>
...
export default {
  ...
  methods: {
    initCanvas() {
      ...
      const w = this.width;
      const h = this.height;
      const PLSize = this.blockSize;
      const padding = this.wraperPadding;
      const MinNX = padding + PLSize;
      const MaxNX = w - padding - PLSize - PLSize / 6;
      const MaxNY = padding;
      const MinNY = h - padding - PLSize - PLSize / 6;
    ...
    render(type, X, Y, d) {
      const canvas = this.$refs[type].getContext('2d');
      if (type === 'puzzleBox') {
    ...
    clearCanvas() {
      ...
      const cl = this.$refs.puzzleShow;
      // let c_s = this.$refs.puzzleShadow;
      c.setAttribute('height', c.getAttribute('height'));
      cl.setAttribute('height', c.getAttribute('height'));
    ...
    moving(e) {
      ...
      const d = moveX - this.moveStart;
      const w = this.width;
      const PLSize = this.blockSize;
      const padding = this.wraperPadding;

      if (this.moveStart === '' || d < 0 || d > w - padding - PLSize + 90) {
        return;
      }
      for (const i of ['sliderBtn', 'puzzleShow']) {
    ...
    moveEnd(e) {
      e = e || window.event;
      const moveEndX = (e.pageX || e.changedTouches[0].pageX) - this.moveStart;
      const verNum = this.randomX - 10;
      const deviationValue = this.deviation;
      const Minleft = verNum - deviationValue;
      const Maxleft = verNum + deviationValue;

      const verResult = Maxleft > moveEndX && moveEndX > Minleft;
