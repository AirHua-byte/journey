<template>
  <div class="container article-list">
    ...
    <section class="list">
      ...
      <template v-else>
        <div class="year-list" v-for="(val, key, idx) in list.data" :key="idx">
          <ul class="month-list" v-for="(vals, keys, idxs) in val" :key="idxs">
            <li class="month">
              {{ enMon[Number(keys) - 1] }}, {{ key.slice(1, 5) }}
            </li>
            <ul class="day-list">
              <li
              ...
              >
                <div class="item-l">
                  <div class="img" @click="viewArticle(child_val.id)">
                    <img
                      :src="child_val.image.url || '/image/other/default.jpg'"
                    />
                  </div>
                  <div class="tit">
                    <span @click="viewArticle(child_val.id)">{{
                      child_val.title
                    }}</span>
                    <span
                      >{{ child_val.like }} LIKE /
                      {{ child_val.read }} READ</span
                    >
                  </div>
                </div>
          ...
          .item-l {
            ...
            .tit {
              ...
              span:first-child {
                ...
                font-size: 15px;
                padding-bottom: 5px;
                transition: backgroundColor 0.3s, backgroundPosition 0s;
                &:hover,
                &:focus {
